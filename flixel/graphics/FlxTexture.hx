...
class FlxTexture
{
	...
	/**
	 * Creates and caches FlxTexture object from openfl.Assets key string.
	 * 
	 * @param	Source	openfl.Assets key string. For example: "assets/image.png".
	 * @param	Unique	Ensures that the bitmap data uses a new slot in the cache. If true, then BitmapData for this FlxTexture will be cloned, which means extra memory.
	 * @param	Key		Force the cache to use a specific Key to index the bitmap.
	 * @param	Cache	Whether to use graphic caching or not. Default value is true, which means automatic caching.
	 * @return	Cached FlxTexture object we just created.
	 */
	public static function fromAssetKey(Source:String, Unique:Bool = false, ?Key:String, Cache:Bool = true):FlxTexture
	{
	...
	/**
	 * Creates and caches FlxTexture object from specified Class<BitmapData>.
	 * 
	 * @param	Source	Class<BitmapData> to create BitmapData for FlxTexture from.
	 * @param	Unique	Ensures that the bitmap data uses a new slot in the cache. If true, then BitmapData for this FlxTexture will be cloned, which means extra memory.
	 * @param	Key	Force the cache to use a specific Key to index the bitmap.
	 * @param	Cache	Whether to use graphic caching or not. Default value is true, which means automatic caching.
	 * @return	FlxTexture object we just created.
	 */
	public static function fromClass(Source:Class<BitmapData>, Unique:Bool = false, ?Key:String, Cache:Bool = true):FlxTexture
	{
		var bitmap:BitmapData = null;
		if (!Cache)
		{
	...
	/**
	 * Creates and caches FlxTexture object from specified BitmapData object.
	 * 
	 * @param	BitmapData for FlxTexture object to use.
	 * @param	Unique	Ensures that the bitmap data uses a new slot in the cache. If true, then BitmapData for this FlxTexture will be cloned, which means extra memory.
	 * @param	Key	Force the cache to use a specific Key to index the bitmap.
	 * @param	Cache	Whether to use graphic caching or not. Default value is true, which means automatic caching.
	 * @return	FlxTexture object we just created.
	 */
	public static function fromBitmapData(Source:BitmapData, Unique:Bool = false, ?Key:String, Cache:Bool = true):FlxTexture
	{
		if (!Cache)
		{
	...
	/**
	 * Creates and caches FlxTexture object from specified FlxFrame object.
	 * It uses frame's BitmapData, not the frame.parent.bitmap.
	 * 
	 * @param	FlxFrame to get BitmapData from for FlxTexture object.
	 * @param	Unique	Ensures that the bitmap data uses a new slot in the cache. If true, then BitmapData for this FlxTexture will be cloned, which means extra memory.
	 * @param	Key	Force the cache to use a specific Key to index the bitmap.
	 * @return	Cached FlxTexture object we just created.
	 */
	public static function fromFrame(Source:FlxFrame, Unique:Bool = false, ?Key:String):FlxTexture
	{
	...
	/**
	 * Creates and caches FlxTexture object from specified FlxFramesCollection object.
	 * It uses frames.parent.bitmap as a source for FlxTexture BitmapData.
	 * It also copies all the frames collections onto newly created FlxTexture.
	 * 
	 * @param	FlxFramesCollection to get BitmapData from for FlxTexture object.
	 * @param	Unique	Ensures that the bitmap data uses a new slot in the cache. If true, then BitmapData for this FlxTexture will be cloned, which means extra memory.
	 * @param	Key	Force the cache to use a specific Key to index the bitmap.
	 * @return	Cached FlxTexture object we just created.
	 */
	public static inline function fromFrames(Source:FlxFramesCollection, Unique:Bool = false, ?Key:String):FlxTexture
	{
		return fromGraphic(Source.parent, Unique, Key);
	}
	
	/**
	 * Creates and caches FlxTexture object from specified FlxTexture object.
	 * It copies all the frames collections onto newly created FlxTexture.
	 * 
	 * @param	FlxTexture to get BitmapData from for FlxTexture object.
	 * @param	Unique	Ensures that the bitmap data uses a new slot in the cache. If true, then BitmapData for this FlxTexture will be cloned, which means extra memory.
	 * @param	Key	Force the cache to use a specific Key to index the bitmap.
	 * @return	Cached FlxTexture object we just created.
	 */
	public static function fromGraphic(Source:FlxTexture, Unique:Bool = false, ?Key:String):FlxTexture
	{
		if (!Unique)
		{
			return Source;
		...
		var graphic:FlxTexture = createGraphic(Source.bitmap, key, Unique);
	...
	/**
	 * Generates and caches new FlxTexture object with a colored rectangle.
	 * 
	 ...
	 * @param	Key		Force the cache to use a specific Key to index the bitmap.
	 * @return	The FlxTexture object we just created.
	 */
	public static function fromRectangle(Width:Int, Height:Int, Color:FlxColor, Unique:Bool = false, ?Key:String):FlxTexture
	{
	 ...
	 * 
	 * @param	Bitmap	BitmapData to use as a graphic source for new FlxTexture.
	 * @param	Key		Key to use as a cache key for created FlxTexture.
	 * @param	Unique	Whether new FlxTexture object uses unique BitmapData or not. If true, then specified BitmapData will be cloned.
	 * @param	Cache	Whether to use graphic caching or not. Default value is true, which means automatic caching.
	 * @return	Created FlxTexture object.
	 */
	private static function createGraphic(Bitmap:BitmapData, Key:String, Unique:Bool = false, Cache:Bool = true):FlxTexture
	{
		Bitmap = FlxTexture.getBitmap(Bitmap, Unique);
		var graphic:FlxTexture = null;
		
		if (Cache)
		{
			graphic = new FlxTexture(Key, Bitmap);
			graphic.unique = Unique;
			FlxG.bitmap.addGraphic(graphic);
		}
		else
		{
			graphic = new FlxTexture(null, Bitmap);
		}
	...
	/**
	 * Whether we should destroy this FlxTexture object when useCount become zero.
	 * Default is true.
	 */
	...
	/**
	 * Usage counter for this FlxTexture object.
	 */
	...
	/**
	 * Searches frames collections of specified type for this FlxTexture object.
	 * 
	 ...
	 * @return	Empty frame with specified size which belongs to this FlxTexture object.
	 */
	...
	private function getBitmapFromSystem():BitmapData
	{
		...
		if (newBitmap != null)
		{
			return FlxTexture.getBitmap(newBitmap, unique);
		}
